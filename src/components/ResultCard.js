...
export default function ResultCard({
  ...
  const onClick = async () => {
    const likeRef = doc(database, "like", `${PRDLST_REPORT_NO}`);
    const response = (await getDoc(likeRef)).data();
    if (response) {
      let resAge = response.age;
      resAge.push(user.age);
      let resJob = response.job;
      resJob.push(user.job);
      let resSex = response.sex;
      resSex.push(user.sex);
  ...
  return (
    <div className="px-5 flex gap-2 h-[250px] w-full border-b-2">
      ...
      <div className="flex flex-col justify-center items-center select-none">
        <div className="border-b-2 pb-2 flex items-center justify-between px-2 w-full">
        ...
        <div className="flex flex-col gap-2 w-full">
          <div className="text-[15px] px-2 py-1">{BSSH_NM}</div>
          ...
          <div
            className="bg-purple-200 py-2 text-center rounded-full hover:bg-purple-400 cursor-pointer"
